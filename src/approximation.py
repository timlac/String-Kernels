...
def test():
    ...
    for k in range(10, max_ngrams, 10):
        ...
        similarity = gram_matrix_alignment(n, currentS, X, X,
                                           kerneltypes);  # measure of similarity between gram matrix made by SSK and by approximated version of the kernel
...
def experiment():
    ...
    print('building gram matrix')
    gram_train = create_gram_matrix(n, S, X_train, X_train,
                                    kerneltype='approx')  #: gram_matrix_approx(n, S, X_train, X_train)
